...
/// @notice Immutable per-pool mandatory fee hook and bounded module composition, with open router compatibility.
/// @dev No arbitrary host calls, swaps, delegatecalls, user identity from hookData, or module access to platform funds.
contract FoundationHookV1 is BaseHook {
    ...
    constructor(
        ...
        if (
            address(manager).code.length == 0 || initializer_ == address(0) || token_.code.length == 0
                ...
                || creatorFeeBps_ > T.MAX_CREATOR_BPS || creatorFeeBps_ % 100 != 0 || initialTick_ % T.TICK_SPACING != 0
                || initialTick_ <= TickMath.minUsableTick(T.TICK_SPACING)
    ...
    function previewGrossFees(bool buy, uint256 amount) external view returns (F.Result memory) {
        return F.quoteGross(amount, creatorFeeBps, feeCarry[buy]);
    }

    function previewNetFees(bool buy, uint256 amount) external view returns (uint256, F.Result memory) {
        return F.quoteNet(amount, creatorFeeBps, feeCarry[buy]);
    }
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        if (buy) result = F.quoteGross(gross, creatorFeeBps, feeCarry[buy]);
        else (gross, result) = F.quoteNet(specified, creatorFeeBps, feeCarry[buy]);
    ...
    function _settlement(SwapParams calldata params, BalanceDelta delta) private view returns (Settlement memory s) {
        ...
        if (s.exactInput) {
            ...
            s.fees = F.quoteGross(s.gross, creatorFeeBps, feeCarry[s.buy]);
        } else {
            (s.gross, s.fees) = F.quoteNet(s.buy ? quoteAmount : specified, creatorFeeBps, feeCarry[s.buy]);
        }
