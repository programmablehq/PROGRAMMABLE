...
import { FoundationPositionVaultV1 } from "./FoundationPositionVaultV1.sol";

interface IFoundationUniversalRouterV1 {
    function execute(bytes calldata commands, bytes[] calldata inputs, uint256 deadline) external payable;
    function poolManager() external view returns (IPoolManager);
}

interface IFoundationPositionPermit2V1 {
    function permit2() external view returns (IAllowanceTransfer);
}

/// @notice Atomic standard-token launch using official Core, PositionManager and UR 2.1.1.
/// @dev Creator supplies no quote LP by default. Optional quote-only opposing band is a separate creator-owned NFT.
contract FoundationFactoryV1 is ReentrancyGuardTransient {
    using SafeERC20 for IERC20;
    using PoolIdLibrary for PoolKey;
    bytes32 public constant VERSION_ID = T.ABI_ID;
    uint256 public immutable chainId;
    IPoolManager public immutable poolManager;
    IPositionManager public immutable positionManager;
    IFoundationUniversalRouterV1 public immutable universalRouter;
    IAllowanceTransfer public immutable permit2;
    FoundationHookDeployerV1 public immutable hookDeployer;
    ...
    mapping(address => T.LaunchResult) private _launches;
    ...
    constructor(
        ...
        IFoundationUniversalRouterV1 router,
        ...
        if (
            address(positions.poolManager()) != address(manager) || address(router.poolManager()) != address(manager)
                || address(IFoundationPositionPermit2V1(address(positions)).permit2()) != address(permits)
        ) {
            revert InvalidInfrastructure();
        }
    }

    function launch(T.LaunchParams calldata p) external nonReentrant returns (T.LaunchResult memory result) {
        _verifyInfrastructure();
        ...
        FoundationHookV1 hook = hookDeployer.deploy(
            poolManager, result.token, p.quote, msg.sender, p.initialTick, p.creatorFeeBps, p.modules, p.hookSalt
        );
        ...
        result.baseVault =
            address(new FoundationPositionVaultV1(positionManager, result.basePositionId, msg.sender, key));
        _mintPosition(key, base, result.baseVault, result.basePositionId, p.deadline);
        uint256 dust = IERC20(result.token).balanceOf(address(this));
        if (dust != T.TOKEN_SUPPLY - base.debt) revert InvalidSettlement();
        if (dust != 0) IERC20(result.token).safeTransfer(result.baseVault, dust);
        if (p.additionalQuoteAmount != 0) {
            result.creatorPositionId = positionManager.nextTokenId();
            _mintPosition(key, additional, msg.sender, result.creatorPositionId, p.deadline);
        }
        ...
        if (remaining > quoteBefore) _transferExact(quoteAsset, msg.sender, remaining - quoteBefore);
        ...
        emit FoundationLaunched(
            ...
            p.quote,
            result.baseVault,
            result.basePositionId,
            result.creatorPositionId,
    ...
    function launchOf(address token) external view returns (T.LaunchResult memory) {
    ...
    function hookInitCodeHash(address creator, address predictedToken, T.LaunchParams calldata p)
        public
        view
        returns (bytes32)
    {
        return hookDeployer.initCodeHash(
            poolManager, address(this), predictedToken, p.quote, creator, p.initialTick, p.creatorFeeBps, p.modules
        );
    }

    function predictHookAddress(address creator, address predictedToken, T.LaunchParams calldata p)
        external
        view
        returns (address)
    {
    ...
    function _mintPosition(PoolKey memory key, PositionPlan memory p, address owner, uint256 id, uint256 deadline)
        private
    {
        ...
        if (
            currency.balanceOf(address(this)) != beforeFactory - p.debt
                ...
                || positionManager.nextTokenId() != id + 1 || IERC721(address(positionManager)).ownerOf(id) != owner
    ...
    function _initialBuy(PoolKey memory key, T.LaunchParams calldata p) private returns (uint256 output) {
