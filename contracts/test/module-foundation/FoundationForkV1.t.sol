...
import { FoundationPositionVaultV1 } from "../../src/module-foundation/FoundationPositionVaultV1.sol";
import {
...
/// @notice Fork simulation only: canonical deployed periphery bytecode is unchanged. Fixtures/funding are local.
abstract contract FoundationForkBase is Test {
    ...
    FoundationFactoryV1 internal factory;
    ...
    function _launch(T.LaunchParams memory p) internal returns (T.LaunchResult memory r) {
        vm.prank(ALICE);
        r = factory.launch(p);
    }

    function tradeExternal(T.LaunchResult memory r, bool buy, bool exactIn, uint256 amount) external returns (uint256) {
        return _trade(r, buy, exactIn, amount);
    }

    function _trade(T.LaunchResult memory r, bool buy, bool exactIn, uint256 amount) internal returns (uint256 other) {
        ...
        IFoundationUniversalRouterV1(ROUTER).execute(hex"10", inputs, block.timestamp + 120);
        vm.stopPrank();
    ...
    function _assertClean(T.LaunchResult memory r) internal view {
...
contract FoundationForkV1Test is FoundationForkBase {
    using StateLibrary for IPoolManager;
    using PoolIdLibrary for PoolKey;

    function test_atomicLaunchZeroModulesBothOrdersInitialBuyAndQuoteOnlyCreatorNFT() public {
        this.checkLaunchExample(true, 0);
        this.checkLaunchExample(false, 2 ether);
    }

    function checkLaunchExample(bool quote0, uint128 extra) external {
        T.LaunchParams memory p = _params(quote0, 0, extra, 1 ether);
        address predicted = factory.predictTokenAddress(ALICE, p.tokenSalt, p.metadata);
        uint256 aliceBefore = quote.balanceOf(ALICE);
        vm.prank(ALICE);
        T.LaunchResult memory r = factory.launch(p);
        ...
        assertEq(IERC721(POSM).ownerOf(r.basePositionId), r.baseVault);
        assertEq(IERC721(POSM).getApproved(r.basePositionId), address(0));
        if (p.additionalQuoteAmount == 0) {
            assertEq(r.creatorPositionId, 0);
        } else {
            assertEq(IERC721(POSM).ownerOf(r.creatorPositionId), ALICE);
            assertGt(positions.getPositionLiquidity(r.creatorPositionId), 0);
            vm.prank(ALICE);
            IERC721(POSM).transferFrom(ALICE, BOB, r.creatorPositionId);
            assertEq(IERC721(POSM).ownerOf(r.creatorPositionId), BOB);
        }
        assertLe(aliceBefore - quote.balanceOf(ALICE), uint256(p.additionalQuoteAmount) + p.initialBuyQuoteAmount);
        assertGe(aliceBefore - quote.balanceOf(ALICE), p.initialBuyQuoteAmount);
        assertEq(FoundationLedgerV1(r.ledger).platformReceived(), uint256(p.initialBuyQuoteAmount) * 30 / 10_000);
        assertEq(FoundationLedgerV1(r.ledger).creatorReceived(), 0);
        assertEq(FoundationHookV1(r.hook).moduleCount(), 0);
        _assertClean(r);
    }
    ...
    function test_realURFourExactnessDirectionsBothOrdersAndCreatorFeeExtremesWithPayout() public {
        for (uint256 order; order < 2; ++order) {
            for (uint256 fee; fee < 2; ++fee) {
                this.checkFourDirections(order == 0, fee == 0 ? 0 : 1000);
            }
        }
    }

    function checkFourDirections(bool quote0, uint16 fee) external {
        T.LaunchResult memory r = _launch(_params(quote0, fee, 0, 0));
    ...
    function test_partialFillExactInputAndOutputRollbackIncludingCarries() public {
        T.LaunchResult memory r = _launch(_params(true, 1000, 0, 0));
        FoundationHookV1 hook = FoundationHookV1(r.hook);
    ...
    function test_tokenBurnOwnershipAndMetadataNoMintAuthority() public {
        T.LaunchResult memory r = _launch(_params(false, 0, 0, 1 ether));
    ...
    function test_baseNFTCannotTransferApproveWithdrawAndFeeCollectRemainsAvailable() public {
        T.LaunchResult memory r = _launch(_params(false, 0, 0, 1 ether));
        uint128 liquidity = positions.getPositionLiquidity(r.basePositionId);
        vm.prank(ALICE);
        vm.expectRevert();
        IERC721(POSM).transferFrom(r.baseVault, ALICE, r.basePositionId);
        vm.prank(ALICE);
        vm.expectRevert();
        IERC721(POSM).approve(ALICE, r.basePositionId);
        bytes[] memory params = new bytes[](1);
        params[0] = abi.encode(r.basePositionId, uint256(1), uint128(0), uint128(0), bytes(""));
        vm.prank(ALICE);
        vm.expectRevert();
        positions.modifyLiquidities(
            abi.encode(abi.encodePacked(uint8(Actions.DECREASE_LIQUIDITY)), params), block.timestamp
        );
        FoundationPositionVaultV1(r.baseVault).collectFees(block.timestamp);
        assertEq(positions.getPositionLiquidity(r.basePositionId), liquidity);
        assertEq(IERC721(POSM).ownerOf(r.basePositionId), r.baseVault);
    }

    function test_taxedFundingAndSlippageDeadlineRollbackBeforeAnyLaunch() public {
        T.LaunchParams memory p = _params(true, 0, 0, 1 ether);
        address token = factory.predictTokenAddress(ALICE, p.tokenSalt, p.metadata);
        uint256 next = positions.nextTokenId();
        quote.setTax(true);
        vm.prank(ALICE);
        vm.expectRevert(FoundationFactoryV1.InvalidSettlement.selector);
        factory.launch(p);
        quote.setTax(false);
        ...
        vm.prank(ALICE);
        vm.expectRevert(FoundationFactoryV1.DeadlineExpired.selector);
        factory.launch(p);
    ...
    function test_statefulCompatibleCompositionBudgetsAndLaunchIsolation() public {
        ...
        _mine(p);
        T.LaunchResult memory r = _launch(p);
        ...
        p.modules = _selections(false, 0, 0);
        _mine(p);
        T.LaunchResult memory other = _launch(p);
    ...
    function test_conflictNestedSwapAndFailurePolicies() public {
        ...
        _mine(p);
        T.LaunchResult memory nested = _launch(p);
        vm.expectRevert();
        ...
        _mine(p);
        T.LaunchResult memory openAfter = _launch(p);
    ...
    function test_sixDecimalQuoteBothSortOrdersWithGrossFeeAccounting() public {
        ...
        quote.approve(address(factory), type(uint256).max);
        for (uint256 order; order < 2; ++order) {
            T.LaunchResult memory r = _launch(_params(order == 0, 100, 0, 0));
