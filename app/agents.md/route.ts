...
export function GET() {
  ...
  const content = [
    ...
    "New Custom Hook keys grant custom-launch:create and custom-launch:read. There is no public API for submitting modules. Existing keys keep their original scopes, wallet binding, expiry and chain restrictions. A scope error is not a reason to rotate or broaden a key automatically. Launch history can include requests from other keys and linked wallets in the same account; the key is not isolated to one project. Its saved chain restrictions still apply. The key cannot sign transactions, move funds or change fee recipients.",
    "## Choose the workflow",
    ...
    "For a website outage, preserve the wallet transaction hash and exact source release. Reconcile the canonical receipt before another send. The existing Native management and Engine clients can verify and prepare actions with the original release evidence and an independent RPC; fee claims use the original ledger and entitled wallet. Keep wallet signing separate.",
