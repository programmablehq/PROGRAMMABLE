...
export function buildProgrammableLlmsIndex(): string {
  return [
    "# Programmable",
    "",
    "> Current entry points for Robinhood Custom Launch Plans, Module Mode and source-bound launch indexing. Ethereum and older Robinhood profiles have separate references.",
    "",
    "## Start here",
    "",
    `- [Agent guide](${PROGRAMMABLE_AGENT_ENTRY.guideUrl}): credentials, launch workflows, wallet authority and recovery.`,
    `- [Agent discovery](${PROGRAMMABLE_AGENT_ENTRY.discoveryUrl}): machine-readable workflow links. The primary Robinhood create workflow is customLaunchPlan.`,
    "- [Custom Launch quickstart](https://programmable.market/docs/developers/custom-launch-quickstart): the current path for a new Robinhood token, hook or application, including a shared token and hook contract.",
    ...
    "",
    "- Build a new Robinhood Custom Launch Plan through the API. For an open plan use admissionPolicy programmable.custom-launch-policy.provenance.v1, feeObligations [] and publication.visibility listed. Preflight the exact packed bytes, create with the same bytes and Idempotency-Key, then let the controller wallet review and sign the prepared transaction.",
    "- Discover finalized Robinhood launch projections and index each launch by its verified source version and canonical token identity. Market data and trading support are separate from provenance.",
    "- Configure and launch a Module Mode coin through the website with the connected wallet. There is no public Module Mode create API.",
    "- Verify Ethereum launches through their exact Registry or finalized Router evidence. The Ethereum V3 guide and older Robinhood V4 profiles are separate from new Robinhood Custom Launch Plans.",
    "",
    ...
    "- Active create authorization from a static schema or stable URL. Read the live Custom Launch Plan capabilities before every new request.",
    "- Wallet authorization from preflight, a model, or caller-attested output. An action_required result requires remediation before wallet signing.",
    "- A mandatory platform fee, liquidity, trading route or safety review from a Custom Launch Plan stamp. The plan's feeObligations field and deployed contract behavior must be evaluated separately.",
    "",
    "## Ethereum identity scope",
    "",
    "- Ethereum catalog: verified Classic V3 launches, Registry-verified Custom launches, finalized Router-stamped Custom provenance entries, and the sole official Classic V2 main-token exception.",
    `- Official main token: ${V4_TOKEN_ADDRESS}.`,
    ...
    "",
    "## Current Robinhood machine-readable resources",
    "",
    `- Custom Launch Plan capabilities: GET ${PROGRAMMABLE_AGENT_ENTRY.workflows.customLaunchPlan.capabilities}`,
    ...
    `- Terminal indexer guide: ${PROGRAMMABLE_AGENT_ENTRY.workflows.customLaunchPlan.indexing.guide}`,
    "- Robinhood Explore catalog: https://programmable.market/api/explore/robinhood",
    "- Sitemap: https://programmable.market/sitemap.xml",
    "- Human documentation: https://programmable.market/docs",
    "- Developer documentation: https://programmable.market/docs/developers",
    "- Developer Markdown: https://programmable.market/docs/developers.md",
    "",
    "## Other integration references",
    "",
    `- Historical MultiRole V2 capabilities: ${multiRoleProject.capabilities}`,
    `- Historical MultiRole V2 guide and client: ${multiRoleProject.guide} and ${multiRoleProject.client}`,
    "- MultiRole V2 Native20 and evidence_required admission rules belong to that historical contract context; the 4.1 profile is also separate from new open Custom Launch Plans.",
    `- Robinhood V4.1 history and separate Ethereum V3 documentation: ${customLaunchHumanGuideUrl}`,
    "- Older /v4/chains/4663/custom-launches requests remain readable for history and recovery. New creation uses /v4/chains/4663/custom-launch-plans.",
    "- Ethereum public OpenAPI: https://programmable.market/openapi.json",
    "- Historical Ethereum token lookup: https://programmable.market/api/explore/token?address=0x7987f03462200b3D8A072E02C89A8A41dCB124EE",
    "",
    "## Ethereum provenance evidence reference",
    "",
    ...
    "- This Ethereum provenance reference grants neither API access, launch authorization nor wallet authorization.",
  ].join("\n");
}
...
export function buildProgrammableLlmsFullFallback(): string {
  return [
    ...
    "",
    "## Expanded Ethereum Router overview",
    "",
