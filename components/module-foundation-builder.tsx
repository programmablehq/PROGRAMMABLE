...
export function ModuleFoundationBuilder({ availability, contextKey, catalog, quoteAssets, onResolveQuote, onUploadImage, onPrepareLaunch, onConfirmLaunch, onRefreshResult, onBack, walletAction, initialDraft, submissionBlocked }: ModuleFoundationBuilderProps) {
  ...
  const [quoteLookup, setQuoteLookup] = useState<{ address: string; status: "checking" | "resolved" | "error"; asset?: FoundationQuoteAsset; message?: string } | null>(null);
  ...
  const [extraLiquidity, setExtraLiquidity] = useState(Boolean(initialDraft?.additionalLiquidity && /[1-9]/.test(initialDraft.additionalLiquidity)));
  ...
  const knownQuote = quoteAssets.find(asset => asset.address.toLowerCase() === draft.quoteAsset.toLowerCase() && asset.chainId === availability.chainId);
  const lookedUpQuote = quoteLookup?.address.toLowerCase() === draft.quoteAsset.toLowerCase() && quoteLookup.status === "resolved" ? quoteLookup.asset : undefined;
  ...
  async function resolveQuote() {
    ...
    setQuoteLookup({ address, status: "checking" });
    try {
      ...
      setQuoteLookup({ address, status: "resolved", asset });
      setAnnouncement(asset.supported ? `${asset.symbol} is supported for this launch.` : asset.reason ?? "This quote token is not supported.");
    } catch (caught) { if (active.current && request === quoteGeneration.current) setQuoteLookup({ address, status: "error", message: cleanError(caught) }); }
  }
  ...
  function validate(): { errors: Errors; links: ModuleSocialLinks } {
    ...
    if (quote) {
      ...
      const valuationError = foundationDecimalError(draft.startValuationQuote, quote.decimals, false);
      const liquidityError = foundationDecimalError(extraLiquidity ? draft.additionalLiquidity : "0", quote.decimals, !extraLiquidity);
      if (buyError) next.initialBuy = buyError;
      if (valuationError) next.startValuationQuote = valuationError;
      if (liquidityError) next.additionalLiquidity = liquidityError;
    }
  ...
  async function prepare(event: FormEvent) {
    ...
    try {
      ...
      const prepared = await onPrepareLaunch({ ...draft, name: draft.name.trim(), symbol: draft.symbol.trim().toUpperCase(), description: draft.description.trim(), quoteAsset: quote!.address,
        socialLinks: checked.links, image: savedImage, additionalLiquidity: extraLiquidity ? draft.additionalLiquidity : "0" });
      assertCurrent();
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.layout}>
      <div className={styles.mainColumn}>
        {result && phase === "result" ? <><ModuleFoundationTransactionResult result={result} onRefresh={onRefreshResult ? () => void refreshResult() : undefined} refreshing={refreshing} />{result.status === "reverted" ? <button type="button" className={styles.secondaryButton} onClick={() => { setResult(null); edit(); }}>Return to coin details</button> : null}{error ? <p className={styles.error} role="alert">{error}</p> : null}</> : activeReview ? <ModuleFoundationLaunchReview review={activeReview} contextKey={contextKey} symbol={draft.symbol.trim().toUpperCase()} busy={busy} disabled={unavailable || Boolean(submissionBlocked)} error={error || submissionBlocked || (unavailable ? availability.reason ?? "Launching is temporarily unavailable." : undefined)} onConfirm={() => void confirm()} onEdit={edit} /> : <form ref={form} className={styles.form} noValidate onSubmit={event => void prepare(event)}>
          <fieldset className={styles.fieldset} disabled={locked || imagePreparing}>
            ...
            <section className={styles.formSection} aria-labelledby="foundation-market-heading">
              ...
              <Field label="Quote token" id="foundation-quote" error={errors.quoteAsset}>
                {!customQuote && quoteAssets.length ? <select id="foundation-quote" value={draft.quoteAsset} aria-invalid={Boolean(errors.quoteAsset) || undefined} aria-describedby={errors.quoteAsset ? "foundation-quote-error" : "foundation-quote-help"} onChange={event => { if (event.target.value === "custom") { setCustomQuote(true); update("quoteAsset", ""); } else { update("quoteAsset", event.target.value); update("startValuationQuote", ""); } }}><option value="" disabled>Choose a quote token</option>{quoteAssets.filter(asset => asset.chainId === availability.chainId).map(asset => <option key={asset.address} value={asset.address} disabled={!asset.supported}>{asset.symbol} · {asset.name}{asset.supported ? "" : " · Unavailable"}</option>)}{onResolveQuote ? <option value="custom">Use another ERC20 token</option> : null}</select> : <div className={styles.quoteInput}><input id="foundation-quote" name="quoteAsset" autoComplete="off" spellCheck={false} placeholder="0x…" value={draft.quoteAsset} aria-invalid={Boolean(errors.quoteAsset) || undefined} aria-describedby={errors.quoteAsset ? "foundation-quote-error" : "foundation-quote-help"} onChange={event => { quoteGeneration.current += 1; update("quoteAsset", event.target.value); update("startValuationQuote", ""); }} />{onResolveQuote ? <button type="button" className={styles.secondaryButton} onClick={() => void resolveQuote()} disabled={locked || quoteLookup?.status === "checking"}>{quoteLookup?.status === "checking" ? "Checking…" : "Check token"}</button> : null}</div>}
                <p id="foundation-quote-help" className={styles.help}>{quote?.supported ? `${quote.symbol} · ${quote.decimals} decimals${quote.balance !== undefined ? ` · Balance ${quote.balance}` : ""}` : quote?.reason ?? "Choose a supported ERC20 on this network. All amounts and swap fees use this token."}</p>
                {quoteLookup?.address === draft.quoteAsset && quoteLookup.status === "error" ? <p className={styles.error}>{quoteLookup.message}</p> : null}
              ...
              <Field label="Starting valuation" id="foundation-valuation" error={errors.startValuationQuote} hint="This prices the full supply of 1 billion coins. It is a starting price, not money in the pool."><div className={styles.amountInput}><input id="foundation-valuation" name="startValuationQuote" inputMode="decimal" autoComplete="off" placeholder="Enter an amount" value={draft.startValuationQuote} aria-invalid={Boolean(errors.startValuationQuote) || undefined} aria-describedby={`foundation-valuation-help${errors.startValuationQuote ? " foundation-valuation-error" : ""}`} onChange={event => update("startValuationQuote", event.target.value)} /><span>{quoteSymbol}</span></div>{quote?.suggestedValuationQuote && quote.valuationSourceLabel ? <button className={styles.textButton} type="button" onClick={() => update("startValuationQuote", quote.suggestedValuationQuote!)}>Use {quote.suggestedValuationQuote} {quote.symbol} · {quote.valuationSourceLabel}</button> : null}</Field>
...
function ModuleField({ field, id, value, onChange }: { field: FoundationConfigurationField; id: string; value: string | boolean | undefined; onChange: (value: string | boolean) => void }) {
  ...
  return <Field label={field.label} id={id} hint={field.description} optional={!field.required}>{field.kind === "select" ? <select id={id} value={String(value ?? "")} aria-describedby={field.description ? `${id}-help` : undefined} onChange={event => onChange(event.target.value)}><option value="">Choose an option</option>{field.options?.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <input id={id} value={String(value ?? "")} autoComplete="off" spellCheck={false} inputMode={field.kind === "decimal" ? "decimal" : field.kind === "integer" ? "numeric" : "text"} aria-describedby={field.description ? `${id}-help` : undefined} onChange={event => onChange(event.target.value)} />}</Field>;
}
